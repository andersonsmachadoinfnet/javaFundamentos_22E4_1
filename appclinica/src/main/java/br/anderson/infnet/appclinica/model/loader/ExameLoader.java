...
@Component
public class ExameLoader  implements ApplicationRunner, IArquivoTxt{
	@Autowired
	private ExameService service;
	
	@Override
	public void run(ApplicationArguments args) throws Exception {
		lerDoArq(Constantes.LOADER_ARQ_EXAME);
	}
	...
	public void lerDoArq(String pNomeDoArq)  {
		...
		try {
			...
			while (lLinha != null) {
				...
				if (Constantes.PREFIXO_CLASSE_PROCEDIMENTO_EXAME.equals(lCampos[0])) {
					Exame item = new Exame(lLinha);
	...
	public void salvarNoArq(String pNomeDoArq) {
		...
		try {
	 		...
	 		for(Exame lItem : service.obterLista()) {
